...
custom = roll(3, CustomDice([-5, -4, -3, -2, -1, 0, 0, 0, 1, 2, 3, 4, 5]));
@test abs(sum(custom.Probability) - 100) <= 0.001

# Custom operation

customf = DicePools.customroll(3,
                               CustomDice([-5, -4, -3, -2, -1, 0, 0, 0, 1, 2, 3, 4, 5])) do r
    return sum(abs.(r)) + 2
end
...
beat = beattarget(3, d12; target=10);
@test sum(beat.Probability) - 100 <= 0.001

below = rollunder(3, d12; target=10);
...
tdf = DataFrame(DicePools.roll(3:5, Conan_Dmg));

psum = combine(groupby(tdf, :Conan_Dmg), :Probability => sum);

@test unique(round.(psum[:, 2], digits=2)) == [100.0]

chance = sort!(combine(groupby(tdf, [:Conan_Dmg, :Hit]),
                       :Probability => sum => :Probability), [:Conan_Dmg, :Hit]); # Probabities for each number of dice and number of Hits

@test maximum(chance.Probability) <= 100

# MY0

tdf = DataFrame(DicePools.roll(1:5, MY0_Attr));

psum = combine(groupby(tdf, :MY0_Attr), :Probability => sum);

@test unique(round.(psum[:, 2], digits=2)) == [100.0]

chance = sort!(combine(groupby(tdf, [:MY0_Attr, :Success]),
                       :Probability => sum => :Probability), [:MY0_Attr, :Success]) # Probabities for each number of dice and number of Hits

@test maximum(chance.Probability) <= 100

cumchance = transform(groupby(chance, [:MY0_Attr]),
                      [:Success, :Probability] => ((s, p) -> sum(p .* [s .<= i for i in s])) => :MoreThan)
